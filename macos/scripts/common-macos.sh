...
EXPECTED_TRAE_TEAM_ID="${TRAE_EXPECTED_TEAM_ID:-79M8227NKH}"
SKIN_VERSION="0.2.0"
...
is_supported_trae_bundle_id() {
  case "$1" in
    com.trae.solo.app|com.trae.work.app) return 0 ;;
    *) return 1 ;;
  esac
}
...
discover_trae_app() {
  ...
  if [ -z "${TRAE_BUNDLE:-}" ]; then
    for identifier in com.trae.solo.app com.trae.work.app; do
